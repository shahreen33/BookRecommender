...
model = load_model("withGRlinks13kdata.h5")
...
def get_CP(AC, bookId):
	...
	if(os.path.isfile(nowBook) == False):
		return -1
...
def get_minimum_req(handle,UP, BookId):
	...
	recommend = get_rating(handle,UP, LocalDB)
	return recommend
...
minimum_req = []
accurate = [0]*len(account_list)
closely_accurate = [0]*len(account_list)
inaccurate = [0]*len(account_list)
accuracy = [0]* len(account_list)

cnt = 1
actual_ratings= []
predicted_ratings_rounded = []
predicted_ratings_not_rounded = []
for i in range(len(account_list)):
	...
	if(temp[1] != '0'):
		...
		(UP, w_count) = UPMaker.MakeUP1(tw_content)
		print(len(UP))
		if(w_count >= 20000 and np.array_equal(np.array(UP), np.array(zero)) == False):
			PR = get_minimum_req(account_list[i],UP,temp[0])				
			predicted_rating = round(PR)
			actual_ratings.append(actual_rating)
			predicted_ratings_rounded.append(predicted_rating)
			predicted_ratings_not_rounded.append(PR)
			print(actual_rating, predicted_rating)
			if(predicted_rating != -1):
				if(predicted_rating == actual_rating):
					accurate[i] +=1
				elif(abs(predicted_rating - actual_rating) == 1):
					closely_accurate[i] += 1
				else:
					inaccurate[i] += 1
				user_total += 1
				cnt += 1
...
#print(accuracy)
print(accurate)
print(closely_accurate)
print(inaccurate)
cnt_accurate = 0
cnt_cl_accurate = 0
cnt_inaccurate = 0
for i in range(len(accurate)):
	cnt_accurate += accurate[i]
	cnt_cl_accurate += closely_accurate[i]
	cnt_inaccurate += inaccurate[i]
total = cnt_accurate +cnt_cl_accurate + cnt_inaccurate
FP = open(str(os.getcwd())+"/ExperimentResults/Experiment1_withGR_NEW",'a')
FP.write("New Experiment:\n\n\n")
FP.write("actual ratings: "+str(actual_ratings)+'\n')
FP.write("predicted ratings rounded: "+str(predicted_ratings_rounded)+'\n')
FP.write("predicted ratings_not_rounded: "+str(predicted_ratings_not_rounded)+'\n')
print(total)
print("Accuracy:", cnt_accurate*100/total )
print("Close Accuracy:", cnt_cl_accurate*100/total )
print((cnt_accurate + cnt_cl_accurate)*100/total)
