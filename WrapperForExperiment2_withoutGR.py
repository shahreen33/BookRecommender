...
model = load_model("Initial_run.h5")
...
def get_custom_tw_profile(account_name, date):
	fname = str(os.getcwd())+"/UserRawData/"+ account_name+"_withoutGR.csv"
	tw_content = ""
...
def get_cluster_members(Cluster_No):
	cluster = str(os.getcwd())+'/'+ "Clusters/withoutGR/Cluster-"+str(Cluster_No)+".txt"
	accounts = []
...
def ClusterUP(handle, UP):
	...
	for i in range(NUM_CLUSTERS):
		cluster = str(os.getcwd())+'/'+ "Clusters/withoutGR/Cluster-"+str(i)+"-Centroid.txt"
		fp = open(cluster, 'rb')
...
def get_recommended_books(handle, UP, DB):
	...
	recommended_books = sorted(result_tupple, key=lambda tup: tup[1])
